...
final class LibraryStore: @unchecked Sendable {
    ...
    /// Open or upgrade the SQLite handle to read-write mode. The default `openIfNeeded`
    /// opens read-only; this is called only when an opt-in write tool runs. We close
    /// the read-only handle first so SQLite's WAL state stays coherent.
    private func openReadWriteIfNeeded() -> Bool {
        if let db {
            sqlite3_close(db)
            self.db = nil
        }
        ...
        guard sqlite3_open_v2(url.path, &newDb, flags, nil) == SQLITE_OK else {
            FileHandle.standardError.write(
                "sqlite3_open_v2 (rw) failed for \(url.path)\n".data(using: .utf8)!
            )
            return false
        }
        self.db = newDb
    ...
    /// Per design §8: writes use a "tiny INSERT OR IGNORE protocol" against the live
    /// store. We atomically bump `Z_PRIMARYKEY.Z_MAX` for the annotation entity, take
    /// the next `Z_PK`, and insert with `Z_OPT = 1`. We DON'T write to Core Data's
    /// persistent-history tables — that format isn't documented and getting it wrong
    /// could corrupt the store — so the app sees the new annotation only on next
    /// launch / window-open (or after a `ModelContext.refresh()`). Annotations are
    /// batch-add use cases, so this is acceptable.
    ///
    /// Returns the inserted annotation's UUID on success, nil on failure.
    func addAnnotation(rigProfileId: String,
                       nightUUID: String?,
                       eventDate: Date,
                       label: String,
                       detail: String,
                       categories: [String],
                       isRigMutating: Bool) -> String? {
        guard openReadWriteIfNeeded(), let db else { return nil }

        // 1. Reserve the next Z_PK atomically. The Z_ENT for AnnotationEntity was 1
        //    in the sample we inspected; we look it up by name so an alphabetical
        //    schema change doesn't silently corrupt rows.
        var entRow: Int32 = 0
        do {
            let sql = "SELECT Z_ENT FROM Z_PRIMARYKEY WHERE Z_NAME = 'AnnotationEntity' LIMIT 1"
            var stmt: OpaquePointer?
            guard sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK else { return nil }
            defer { sqlite3_finalize(stmt) }
            guard sqlite3_step(stmt) == SQLITE_ROW else { return nil }
        ...
        do {
            ...
            defer { sqlite3_finalize(stmt) }
            sqlite3_bind_int(stmt, 1, entRow)
            guard sqlite3_step(stmt) == SQLITE_ROW else {
                sqlite3_exec(db, "ROLLBACK", nil, nil, nil)
                return nil
            }
            newPK = sqlite3_column_int64(stmt, 0)
        }
        ...
        if let nightUUID {
            ...
            if sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK {
                if bindUUID(stmt, idx: 1, uuid: nightUUID), sqlite3_step(stmt) == SQLITE_ROW {
        ...
        // 3. Insert the row.
        let insertSQL = """
        INSERT INTO ZANNOTATIONENTITY
          (Z_PK, Z_ENT, Z_OPT, ZID, ZRIGPROFILEID, ZNIGHTRECORD,
           ZEVENTDATE, ZCATEGORIESDATA, ZLABEL, ZDETAIL,
           ZISRIGMUTATING, ZCREATEDAT, ZMODIFIEDAT)
        VALUES (?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        var stmt: OpaquePointer?
