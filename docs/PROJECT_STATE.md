...
> Generated from commit 3e5dd4b at version 2.0 / build 1, 2026-08-01.

<!-- sidecar:facts:begin -->
> **Measured at `3e5dd4b` (`v2`), 2026-08-01:** 91 Swift files in the app target `Ephemeris/`; 17,840 lines of Swift; 23 test files.
<!-- sidecar:facts:end -->

*Snapshot for design / strategy reference. Generated from the codebase on the `v2` branch and not from memory. **2.0 shipped 2026-07-31**: `v2` was merged to `main`, tagged `v2.0`, and published to GitHub Releases with a Sparkle appcast; everything below describes the shipped 2.0 plus the post-release work on the branch. See `CHANGELOG.md` and `RELEASE_NOTES.md`; deferred work lives in `ROADMAP.md`. Branch topology since the ship: `main` and `v2` point at the same commit, and releases are cut from `main` (`RELEASE.md`). For an outside reader: this is an open-source (GPLv3) project, and this document plus the README are the honest picture of where it stands. Supersedes the v1 snapshot; see `docs/ephemeris-2.0-design-document.md` for the design the v2 build followed.*
...
- **An embedded MCP server** — a loopback-only HTTP listener inside the app exposing read-only library tools, bearer-token gated by default (plus a bundled stdio helper binary with a one-click Claude Desktop / Claude Code installer, and a documented tunnel recipe for ChatGPT connectors: `docs/chatgpt-connector.md`).
...
- **Sandbox / entitlements:** app-sandboxed; `files.user-selected.read-write` plus `network.server` (for the loopback MCP listener), and the Sparkle sandboxed-install mach-lookup exceptions (`-spks`/`-spki`, alongside `SUEnableInstallerLauncherService`) so in-app installs work under the sandbox. Security-scoped folder bookmarks (`SourceFolderBookmarks`) persist access to log folders.
...
**MCP layer (`MCPServer/`).** `MCPEmbeddedServer` (NWListener on 127.0.0.1, dynamic port persisted in UserDefaults, bearer token generated on first use and persisted, status state machine), `MCPConnectionHandler` (HTTP request handling, origin hardening, bearer-token check), `MCPProtocolHandler` (JSON-RPC), `MCPTools` (tool registry reading the library).
...
- **`RecommenderObservation`** — scope (`singleNight`/`crossNight`), category (subQuality → opticalTrain → equipment → phd2Hygiene → pattern → suggestion, which is also the triage order), severity (alert → coaching), confidence, `SourceAuthority`, title/summary/suggestedResponse, evidence items, candidate contributors, related `PHD2Tool`s and `HelpTopic`s, and `sessionStartedAt` (anchors a finding to its source guide session when the evidence comes from exactly one session; nil for night-level findings).
...
- **`ObservationEntity`** — persisted recommender output (enums stored raw, evidence/contributors/help-topics as JSON), `dismissedAt` for user dismissal, `sessionStartedAt` mirroring the value type's session anchor.
...
1. **`DocumentGroup`** (read-only `GuideLogDocument` → `ContentView`) — the v1 single-log viewer, lightly enhanced: observation cards in the inspector (data-first layout with the session's numbers up top; observations collapsed by default, and session-anchored findings shown only under their own session), auto-ingest on open, commands for Log Library (⇧⌘L), Rig Profiles, MCP Server, Import Folder.
...
4. **`Window` "MCP Server"** (`id: mcpServer`, 640×620) — server status/port, connection stats, restart, bearer-token display and require-token toggle, one-click Claude Desktop / Claude Code installers, manual-config fallback display.
...
`RecommenderEngine` registers **21 single-night generators** in three authority tiers, and **4 cross-night generators** run during ingest against the rig's corpus. Output is sorted by category then severity; heuristic-tier findings use softened, non-imperative voice per the design's voice rules.
...
**Gap-analysis tier** (`Generators/GapAnalysisObservers.swift`, closing the matrix in `docs/observation-gap-analysis.md`): StarShapePrediction, PierSideBias, CooldownSignature, MinMoveValidation, GuideRateValidation, Aggressiveness, DataDrivenAlgorithmHint, GuideScaleMismatch, StarLost, plus the July 31 forum-triage pair modeled on the PHD2 maintainers' actual forum responses: SettlingFailure (repeated post-dither settle failures pointing at polar alignment / Dec backlash / settle tolerance) and GuideExposureClass (guide-exposure sanity for the rig's mount class).
...
**Periodicity gating (forum-triage pass).** `FrequencySpectrum.prominentPeriod` reports a dominant period only when the peak is genuinely prominent (energy concentration plus enough full cycles in the data span), unlike `dominantPeriod`, which is chart-label-only. The Aggressiveness observer uses it as a guard: a genuinely periodic residual reframes the card as a mechanical/periodic-error finding (balance, worm mesh, PredictivePEC) instead of tuning advice, and the PredictivePEC hint observer gates on it too.
...
**Embedded (primary).** `MCPEmbeddedServer` runs inside the app — Network.framework listener bound to 127.0.0.1 only, dynamic port persisted across launches, JSON-RPC over HTTP POST. Since the 2026-07-31 hardening pass, POST `/mcp` requires a bearer token by default: the token is generated on first use, persisted, shown in the MCP Server window, and requests without it get 401 plus a `WWW-Authenticate: Bearer` header (the user can turn the requirement off; doing so restores the previous any-local-process exposure). Hardened against browser cross-origin probing and malformed input (a supplied-but-invalid `rig_id` is an error, not a fall-through to "all rigs"; `list_nights` date/limit are bound as SQL parameters; requests are capped at 1 MB with strict Content-Length validation, answered with 400/413 instead of buffering forever), with MCP spec-compliance fixes from the same pass. Seven read-only tools: `list_rigs`, `list_nights`, `list_observations`, `get_aggregate_stats`, `get_corpus_summary`, and the ChatGPT-connector pair `search` / `fetch` (ChatGPT custom and deep-research connectors only reach public HTTPS MCP servers; `docs/chatgpt-connector.md` documents the tunnel recipe that makes the loopback server reachable, which is exactly why the bearer gate exists). The per-night rollup is exposed as `night_rms_arcsec` in both catalogs (true across-nights medians keep their `median` names). Source-authority labels survive into tool output so an AI consumer can distinguish PHD2-measured facts from Ephemeris heuristics.

**Stdio helper (for clients that can't speak HTTP).** `tools/ephemeris-mcp/` — a separate SPM executable bundled into `Ephemeris.app/Contents/Helpers/`, reading the library store directly (read-only SQLite). Same library tool surface as the embedded server, and strictly read-only: the former opt-in `add_annotation` write tool, its raw-SQLite write path, and the `EPHEMERIS_MCP_ALLOW_WRITES` env gate were all removed, and the helper package's own test target asserts catalog parity plus the no-writes guarantee. `ClaudeConfigInstaller` does the one-click setup: NSOpenPanel grant → JSON-merge of the `ephemeris` entry into the Claude Desktop / Claude Code config, idempotent and atomic, keeping a `.bak` copy of the pre-install config and writing no env block; its hardcoded dev-path fallback for the helper binary is DEBUG-only.
...
charts, icon buttons, and combined rows. The release-eve pass on 2026-07-31 fixed
the Sparkle sandboxed install and closed the embedded server's no-auth-token gap
with the default-on bearer requirement.)*

- **Sparkle is live** — the EdDSA keypair exists (private key in
  the signing Mac's login Keychain, public key in Info.plist), and the sandboxed
  install works as of 2026-07-31 (`SUEnableInstallerLauncherService` plus the
  `-spks`/`-spki` mach-lookup entitlements; `SUEnableAutomaticChecks` is
  deliberately absent from Info.plist so Sparkle's own consent prompt runs on
  second launch and stores the user's choice). Each release still needs an
  `appcast.xml` asset (see docs/RELEASE.md).
...
- **The MCP bearer requirement is user-disableable** — the server stays opt-in,
  loopback-only, and read-only, but a user who turns the token requirement off
  restores the old exposure (any local process can read library summaries).
...
**Tests:** `xcodebuild … test`. The unit suite covers: parser + signature + calibration headers (including malformed-field counting), session/aggregate stats, FFT and spectrum-prominence gating, merger, CSV, rig profiles and the SwiftData-backed store, annotations, the recommender engine (single-night and cross-night generators, threshold boundaries, the forum-triage observers), observation pair merging, chart decimation (`GuideChartData`), the embedded MCP server (request hardening, malformed input, bearer-token auth, the search/fetch pair), library ingest idempotency, and a corpus-validation pass that parses every bundled real log and asserts the recommender fires (skips gracefully when the corpus folder is absent). The stdio helper package has its own test target (`tools/ephemeris-mcp/Tests/`) asserting catalog parity with the embedded server and the read-only guarantee.
...
Ephemeris/
  ...
  Recommender/     RecommenderEngine, SingleNightContext, CrossNightContext,
    ...
    Generators/    PHD2CanonGenerators (8), HeuristicGenerators (2),
                   GapAnalysisObservers (11), CrossNightGenerators (4)
...
docs/
  ...
  chatgpt-connector.md               ChatGPT custom-connector recipe (tunnel +
                                     bearer token for the embedded MCP server)
